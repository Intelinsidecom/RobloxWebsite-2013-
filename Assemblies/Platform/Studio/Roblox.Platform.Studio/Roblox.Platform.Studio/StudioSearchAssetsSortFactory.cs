...
// Use alias to pin to Platform IAsset
using IAsset = Roblox.Platform.Assets.IAsset;
 

namespace Roblox.Platform.Studio;

public class StudioSearchAssetsSortFactory : IStudioSearchAssetsSortFactory, IDomainFactory<StudioDomainFactories>, IDomainObject<StudioDomainFactories>
{
    ...
    {
        if (assetsFromWilsonScoreSearch == null || assetsFromWilsonScoreSearch.Count == 0)
        {
            return assetsFromDefaultSearch?.ToList() ?? new List<IAsset>();
        }
        if (assetsFromDefaultSearch == null || assetsFromDefaultSearch.Count == 0)
        {
            return assetsFromWilsonScoreSearch.ToList();
        }
        // Fallback simple merge without Id/comparer dependence.
        return assetsFromWilsonScoreSearch
            .Concat(assetsFromDefaultSearch)
            .Distinct()
            .ToList();
    }
    ...
    {
        // For now, use the same simple merge regardless of settings to ensure correct typing.
    ...
    private void CalculateMeanSuccessFailure(IEnumerable<Roblox.Platform.Assets.IAsset> assets, IAssetSearchKeywordStatisticFactory assetSearchKeywordStatisticFactory, string keyword, double shrink, out double meanSuccess, out double meanFailure)
    {
        meanSuccess = 0.0;
        meanFailure = 0.0;
        double success = 0.0;
    ...
    private double GetShrink(IEnumerable<Roblox.Platform.Assets.IAsset> assets, IAssetSearchKeywordStatisticFactory assetSearchKeywordStatisticFactory, string keyword)
    {
    ...
    public ICollection<IAsset> GetSortedAssets(ICollection<IAsset> assetsFromFirstSearchQueryResults, ICollection<IAsset> assetsFromSecondSearchQueryResults, int firstSearchQueryCount, int secondSearchQueryCount, IAssetSearchKeywordStatisticFactory assetSearchKeywordStatisticFactory, string keyword, int maxResults)
    {
        ...
        if (firstSearchQueryCount + secondSearchQueryCount == maxResults)
        {
            ...
            IEnumerable<IAsset> second = assetsFromSecondSearchQueryResults.Take(secondSearchQueryCount);
            // Simple concatenation with Distinct to avoid duplicates without custom comparer.
