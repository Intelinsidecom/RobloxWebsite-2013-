...
{
    private readonly Client.ITranslationStorageClient _TranslationStorageClient;
    ...
    public GenericTranslationStorageAccessor(Client.ITranslationStorageClient translationStorageClient, ILogger logger, Func<bool> isAccessingTranslationsEnabledGetter, Func<bool> isReturningTranslationsEnabledGetter, Func<int> shadowRolloutPercentageGetter)
    {
    ...
    public string GetTranslationValue(ValueSource<TIdentifier> valueSource, string contentSourceType, string contentVariantType, string contentLocaleType)
    {
        ...
        try
        {
            Client.GetTranslationResponse translation2 = _TranslationStorageClient.GetTranslation(getTranslationRequest);
    ...
    public IReadOnlyDictionary<TIdentifier, string> MultiGetTranslationValues(ICollection<ValueSource<TIdentifier>> valueSources, string contentSourceType, string contentVariantType, string contentLocaleType)
    {
        ...
        Client.GetTranslationsForContentSourceTargetIdsRequest getTranslationsForContentSourceTargetIdsRequest = new Client.GetTranslationsForContentSourceTargetIdsRequest
        {
            ContentLocaleType = contentLocaleType,
            ContentSourceType = new Client.ContentSourceType
            {
        ...
        ICollection<Client.ContentSourceTargetIdTranslationValue> contentSourceTargetIdTranslationValues;
        try
        {
            contentSourceTargetIdTranslationValues = _TranslationStorageClient.GetTranslationsForContentSourceTargetIds(getTranslationsForContentSourceTargetIdsRequest).Translations.Where((Client.ContentSourceTargetIdTranslationValue x) => !string.IsNullOrWhiteSpace(x.TranslationValue)).ToList();
        }
        catch (Exception ex)
        {
            _Logger.Error(ex);
    ...
    public IReadOnlyCollection<TranslationValue> GetTranslationValuesInAllContentLocales(string sourceLocale, string contentSourceType, string contentVariantType, string contentSourceTargetId)
    {
        ...
        ICollection<Client.ContentLocaleTranslationValue> translationValues;
        try
        {
            Client.GetTranslationsForContentLocalesRequest getTranslationsForContentLocalesRequest = new Client.GetTranslationsForContentLocalesRequest
            {
                ContentSourceType = new Client.ContentSourceType
                {
                    Value = contentSourceType
        ...
        foreach (Client.ContentLocaleTranslationValue translationForContentLocale in translationValues)
        {
    ...
    private static bool TryConvertToClientContentVariantType(string contentVariantType, out Client.ContentVariantType clientContentVariantType)
    {
        //IL_001e: Unknown result type (might be due to invalid IL or missing references)
        //IL_0024: Expected I4, but got Unknown
        clientContentVariantType = (Client.ContentVariantType)0;
        if (!string.IsNullOrWhiteSpace(contentVariantType))
        {
            Client.ContentVariantType? parsedEnum = EnumUtils.StrictTryParseEnum<Client.ContentVariantType>(contentVariantType);
            if (parsedEnum.HasValue)
            {
                clientContentVariantType = (Client.ContentVariantType)(int)parsedEnum.Value;
                return true;
            }
        }
        return false;
