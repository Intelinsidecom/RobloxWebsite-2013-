param(
    ...
    # Default to the parent directory of this script's folder (one level up)
...
Write-Host "Target root:" -NoNewline; Write-Host " $rootPath" -ForegroundColor Cyan

# Discover candidates: directories named 'bin', 'obj', or 'packages' (stream findings in real time)
Write-Host "\nSearching for bin/obj/packages folders..." -ForegroundColor Cyan
$candidates = @()
Get-ChildItem -LiteralPath $rootPath -Recurse -Directory -Force |
    Where-Object { $_.Name -in @('bin','obj','packages') } |
    ForEach-Object {
        $candidates += $_
...
if (-not $targets -or $targets.Count -eq 0) {
    Write-Host "No bin/obj/packages folders found to clean (after exclusions)." -ForegroundColor Yellow
    exit 0
}
...
Write-Host "\nThe following bin/obj/packages folders would be deleted:" -ForegroundColor Green
