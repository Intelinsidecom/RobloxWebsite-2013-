...
public class AudioReplacer : IAudioReplacer
{
	...
	private readonly Roblox.Platform.Assets.IRawContentFactory _RawContentFactory;

	private volatile Roblox.Platform.Assets.IRawContent[] _ReplacementAudioSelection;

	/// <inheritdoc cref="M:Roblox.Platform.AudioRightsManagement.IAudioReplacer.ReplaceAudio(Roblox.Platform.Assets.IRawContent)" />
	public event Action<Roblox.Platform.Assets.IRawContent> OnRawContentHasBeenReplaced;
	...
	public AudioReplacer(ILogger logger, Roblox.Platform.Assets.IAssetFactory assetFactory, Roblox.Platform.Assets.IAssetVersionFactory assetVersionFactory, Roblox.Platform.Assets.IRawContentFactory rawContentFactory)
	...
	/// <exception cref="T:System.ArgumentNullException"><paramref name="assetVersionFactory" /></exception>
	/// <exception cref="T:System.ArgumentNullException"><paramref name="rawContentFactory" /></exception>
	internal AudioReplacer(ISettings settings, ILogger logger, Roblox.Platform.Assets.IAssetFactory assetFactory, Roblox.Platform.Assets.IAssetVersionFactory assetVersionFactory, Roblox.Platform.Assets.IRawContentFactory rawContentFactory)
	{
	...
	/// <inheritdoc cref="M:Roblox.Platform.AudioRightsManagement.IAudioReplacer.ReplaceAudio(Roblox.Platform.Assets.IRawContent)" />
	public Roblox.Platform.Assets.IRawContent ReplaceAudio(Roblox.Platform.Assets.IRawContent rawContent)
	{
		if (rawContent == null)
		{
	...
	private void BuildRawContentCache(string assetIdsCsv)
	{
		...
		List<Roblox.Platform.Assets.IRawContent> result = new List<Roblox.Platform.Assets.IRawContent>();
		foreach (string stringId in ids)
		{
			...
			Roblox.Platform.Assets.IAsset asset = _AssetFactory.GetAsset(id);
			if (asset == null)
			{
				_Logger.Error($"Replacement audio asset contained id {id} which is not valid.");
				return;
			}
			Roblox.Platform.Assets.IAssetVersion assetVersion = _AssetVersionFactory.GetCurrentAssetVersion(asset);
			if (assetVersion == null)
			{
				_Logger.Error($"Replacement audio asset contained id {id} which does not have a valid current version.");
				return;
			}
			Roblox.Platform.Assets.IRawContent rawContent = _RawContentFactory.Get(assetVersion.RawContentId);
			if (rawContent == null)
			{
